...
function era_callback(&$out)
{
    ...
    if (isset($_REQUEST['chk' . $chk_123])) {
        ...
        if ($csc == '4' || ($csc == '1' && !$hasPayment)) {
            ...
            if (!$debug) {
                if ($pid && $encounter) {
                    ...
                    foreach ($out['svc'] as $svc) {
                        $billing_id = arGetBillingId($pid, $encounter, $billing_ids_handled, $svc['code'], $svc['mod']);

                        arProcessServiceRemarks($csc, $svc, $billing_id);
                        arProcessServiceAdjustments($csc, $svc, $billing_id);
                    }

                    //This next line returns all but the last character (i.e. removes the last comma)
                    $code_value = $svc['code'].":".$svc['mod']. "($billing_id)";
        ...
        foreach ($out['svc'] as $svc) {
            ...
            $billing_id = arGetBillingId($pid, $encounter, $billing_ids_handled, $code, $mod);
            $codekey = $billing_id;
