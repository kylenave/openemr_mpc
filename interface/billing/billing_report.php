...
if ($daysheet) {
$ThisPageSearchCriteriaDisplayMaster= array( xl("Date of Service"),xl("Date of Entry"),xl("Date of Billing"),xl("Claim Type"),xl("Patient Name"),
                                      xl("Patient Id"),xl("Insurance Company"),xl("Encounter"),xl("Whether Insured"),xl("Charge Coded"),
                                      xl("Billing Status"),xl("Authorization Status"),xl("Last Level Billed"),xl("X12 Partner"),xl("User"), xl("Provider"));
...
$ThisPageSearchCriteriaKeyMaster="form_encounter.date,billing.date,claims.process_time,claims.target,patient_data.fname,".
                                 ...
                                 "billing.authorized,form_encounter.last_level_billed,billing.x12_partner_id,billing.user,form_encounter.provider_id";

$ThisPageSearchCriteriaDataTypeMaster="datetime,datetime,datetime,radio,text_like,".
                                      "text,include,text,radio,radio,radio,".
                                      "radio_like,radio,query_drop_down,text,query_drop_down";
}
else
{
...
$ThisPageSearchCriteriaDisplayMaster= array( xl("Date of Service"),xl("Date of Entry"),xl("Date of Billing"),xl("Claim Type"),xl("Patient Name"),
                                      xl("Patient Id"),xl("Insurance Company"),xl("Encounter"),xl("Whether Insured"),xl("Charge Coded"),
                                      xl("Billing Status"),xl("Authorization Status"),xl("Last Level Billed"),xl("X12 Partner"));
...
$ThisPageSearchCriteriaDataTypeMaster="datetime,datetime,datetime,radio,text_like,".
                                      "text,include,text,radio,radio,radio,".
                                      "radio_like,radio,query_drop_down,include";
...
$ThisPageSearchCriteriaQueryDropDownMaster[2]="SELECT lname as name,id FROM users where authorized='1';";
...
<?php
  ...
  foreach ($ret as $iter) {
    ...
    if ($last_encounter_id != $this_encounter_id) {
      ...
      $ptname = $name['fname'] . " " . $name['lname'] . " ($" . $enc_balance . ")";
      ...
      if ($iter['id']) {
        ...
        $resp_party = ar_responsible_party($iter['enc_pid'], $iter['enc_encounter']);
//echo $iter['enc_pid'] . " " . $iter['enc_encounter'] . "Responsible party = " . $resp_party . "\n";
        
	switch ($resp_party) {
    	    case '1':
		$resp_party_type = 'primary';
	        break;
	    case '2':
		$resp_party_type = 'secondary';
	        break;
	    case '3':
		$resp_party_type = 'tertiary';
	        break;
	    default:
	}
          ...
          if (strcmp($row['type'], $prevtype) == 0) 
	  {
		continue;
          ...
          if (strlen($row['provider']) > 0) 
          {
            ...
            $insuranceTypeOption = substr($row['type'],0,1) . $row['id'];
            
            $lhtml .= "<option value=\"" . attr($insuranceTypeOption) . "\"";

	    if(($resp_party_type) && $row['type'])
	    {
		if($resp_party_type == $row['type'])
		{
			$lhtml .= " selected";
