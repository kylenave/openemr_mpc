...
if($_POST['clear_fu'])
{
   list($clearEncounter,$clearPid) = split(":",$_POST['clear_fu'],2);
}

if($clearEncounter)
{
   echo "Clearing denied flag for encounter: " . $clearEncounter;

   arClearDeniedFlag($clearPid, $clearEncounter);
}
...
<title>New Denied Claims Report</title>
...
<span class='title'>New Denied Claims </span>

<form method='post' name='theform' id='theform' action='denied_claims_2.php' onsubmit='return top.restoreSession()'>
...
select c.encounter_id, c.version, c.patient_id, c.status, c.process_file as reasons, date(bill_time) as last_billed_date from claims c
where c.status='7' and c.version=(select version from claims where encounter_id=c.encounter_id order by version desc limit 1)");
...
");
 ...
 $res = sqlStatement("
select distinct fe.encounter, f.name as facility, doc.lname as provider, p.lname, p.fname, date(fe.date) as dos, cd.reasons, ec.fee, ic.name as payer
...
left join t_claimDenials cd on cd.encounter_id=fe.encounter
left join claims c2 on c2.encounter_id=fe.encounter and c2.version=cd.version-1
left join insurance_companies ic on ic.id=c2.payer_id
left join ar_activity ar on ar.pid=fe.pid and ar.encounter=fe.encounter
where fe.external_id='1' 
...
$clearEnc = $row['encounter'] . ":" . $row['pid'];
