 ...
 */
	...
	$query = "		SELECT distinct
								   ...
								   c.name as payer_name,
                                                                   DATE_FORMAT(fe.date, '%Y%m%d') as pc_eventDate 
                                                        FROM patient_data p 
							LEFT JOIN (select pid, max(date) as date, facility_id from form_encounter group by pid) fe on fe.pid=p.pid 
                                                        left join facility f on f.id=fe.facility_id
							LEFT JOIN users AS d on fe.pid= d.id
							...
							WHERE c.cms_id='MCDIL' and fe.date>='2017-12-01'";
