...
$report_name = "Report Case Log";
$report_file = "report_case_log";
...
<?php
  ...
  if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
    $columns = array("Code"=>"code", "Description"=>"code_text" ,"Count"=>"units", "Billed"=>"billed","Paid"=>"PaidAmount");
    $moneyFormat = array("Code"=>false, "Description"=>false, "Count"=>false, "Billed"=>true, "Paid"=>true);
    $calcGrandTotals = array("Code"=>false, "Description"=>false, "Count"=>true, "Billed"=>true, "Paid"=>true);
    $grandTotals = array("Code"=> 0, "Description"=>0, "Count"=>0, "Billed"=>0, "Paid"=>0);
    ...
    $sqlBindArray = array();
    $query = "select b.code, c.code_text, sum(b.units) as units, sum(b.fee) as billed, sum(ar_act.paid) as PaidAmount, " .
        "sum(ar_act.adjust) as AdjustAmount,(sum(b.fee)-(sum(ar_act.paid)+sum(ar_act.adjust))) as Balance, " .
        "fe.provider_id " .
        "FROM form_encounter as fe " .
        "join (select encounter, code, sum(fee) as fee, sum(units) as units from billing where activity=1 and fee > 0 group by encounter, code) as b on b.encounter=fe.encounter " .
        "left JOIN (select pid,encounter,code,sum(pay_amount) as paid,sum(adj_amount) as adjust from ar_activity group by pid,encounter,code) as ar_act " .
        "ON ar_act.pid=fe.pid and ar_act.encounter=b.encounter and ar_act.code=b.code " .
	" left join codes c on c.code=b.code " .
        "WHERE " .
        "fe.date >=  ? AND fe.date <= ?";
   array_push($sqlBindArray,"$from_date 00:00:00","$to_date 23:59:59");
    ...
    // If a provider was specified.
      ...
      $query .= " GROUP BY b.code ORDER BY b.code ";
