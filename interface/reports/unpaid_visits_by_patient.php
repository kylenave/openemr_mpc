...
$report_name = "Unpaid Visits";
$report_file = "unpaid_visits_by_patient";
...
<?php
  ...
  if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
    $columns = array("Last"=>"lname", "First"=>"fname","DOB"=>"dob", "UnpaidVisits"=>"visits");
    ...
    $sqlBindArray = array();
    $query = "DROP TABLE IF EXISTS t_insPayments";
    sqlStatement($query);

    $query = "create temporary table t_insPayments
(
   primary key encounter (encounter)
)
select fe.encounter, sum(ar.pay_amount) as payments from form_encounter fe
left join ar_activity ar on ar.encounter=fe.encounter
left join ar_session ss on ss.session_id=ar.session_id
where ss.payment_type='insurance' or ar.encounter is null
group by fe.encounter ";
      sqlStatement($query);


      $query="DROP TABLE IF EXISTS t_unpaidVisits";
      sqlStatement($query);

      $query="create temporary table t_unpaidVisits
(
   primary key pid (pid)
)
...
where 
(ip.payments=0 or ip.payments is null) and fe.last_level_closed<1
group by fe.pid";
      sqlStatement($query);

      $query="select p.pid, p.lname, p.fname, p.dob, uv.visits 
from patient_data p
left join t_unpaidVisits uv on uv.pid=p.pid
where uv.visits > 3
order by uv.visits desc";
