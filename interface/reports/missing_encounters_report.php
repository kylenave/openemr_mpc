...
$report_name = " Missing Encounters Report";
$report_file = "missing_encounters_report";
...
<?php
  ...
  if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
    $columns = array("DOS"=>"pc_eventDate", "Last"=>"lname", "First"=>"fname", "DOB"=>"DOB", "Provider"=>"provider", "Facility"=>"facility_code");
    $moneyFormat = array();
    ...
    $sqlBindArray = array();
    $query = "
select apt.pc_pid, p.lname, p.fname, p.DOB, apt.pc_eventDate, doc.lname as provider, f.facility_code, apt.pc_apptstatus, fe.encounter 
from openemr_postcalendar_events apt
left join form_encounter fe on date(fe.date)=apt.pc_eventDate and fe.pid=apt.pc_pid
left join patient_data p on p.pid=apt.pc_pid
left join billing b on b.encounter=fe.encounter
left join users doc on doc.id=apt.pc_aid
left join facility f on f.id=apt.pc_facility " .
        "where apt.pc_apptstatus='@' and (fe.encounter is null or not(fe.encounter is null) and b.code is null) " .
        "and apt.pc_eventDate >=  ? AND apt.pc_eventDate <= ?";
   array_push($sqlBindArray,"$from_date 00:00:00","$to_date 23:59:59");
    // If a facility was specified.
      if ($form_facility) {
        $query .= " AND apt.pc_facility = ?";
