...
function parse_era_2100(&$out, $cb) {
	...
	//We don't like the artificailly inserted 'Claim' item at Atlas so are removing.
        $cb($out);
        return;
/*
  ...
  $totalSegmentCount=0;
    ...
    if (strlen($buffer) < 2048 && ! feof($infh))
    {
       $buffer .= fread($infh, 2048);
        ...
        {
           $inline = trim(substr($buffer, 0, $tpos));
           $buffer = trim(substr($buffer, $tpos + 1));
        }
    ...
    $totalSegmentCount++;
        ...
        //
            ...
            //if ($svc[0] != 'HC') return 'SVC segment has unexpected qualifier: ' . $svc[0];
      ...
      $out['warnings'] .= "$inline at service level ignored.\n";
            ...
            if(trim($segid)!='')
            return "Unknown or unexpected segment ID [$totalSegmentCount : $segid : $inline]";
        }

        ++$out['st_segment_count'];
    }
    if ($segid != 'IEA') return 'Premature end of ERA file:['. $segid .']';
    return '';
}
