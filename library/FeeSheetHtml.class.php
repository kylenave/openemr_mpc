...
class FeeSheetHtml extends FeeSheet {
  ...
  public static function genProviderOptionList($toptext, $default=0) {
    ...
    {
        $restrict_provider_facility = $GLOBALS['gbl_restrict_provider_facility'];
    }
    
   

    if ($restrict_provider_facility) {
    ...
    while ($row = sqlFetchArray($res)) {
      ...
      $s .= ">";
      if (!$restrict_provider_facility && $def_facility && $row['facility_id'] == $def_facility) {
