...
class Claim {
  ...
  var $diagtype= "ICD10";  // diagnosis code_type.Assume ICD9 unless otherwise specified.
  ...
  function loadPayerInfo(&$billrow) {
    ...
    //This kludge is specific to Illinois Medicare and DME
      ...
      $crow = sqlQuery("SELECT * FROM insurance_companies WHERE id='244'");
      $orow = new InsuranceCompany('244');

      $this->mcr_dme_payer['data'] = $drow;
      $this->mcr_dme_payer['company'] = $crow;
      $this->mcr_dme_payer['object'] = $orow;
...
//error_log("UDS Claim");
    ...
    {
       $sql = "SELECT * FROM x12_partners WHERE " .
              "id = '" . $this->payers[0]['company']['x12_default_partner_id'] . "'";
       $this->x12_partner = sqlQuery($sql);
    }

    //ADJUST THIS FOR JEFFRIES AND SOTO AT CHICAGO UNTIL CONTRACTING AND CREDENTIALING IS FIXED (KBN)
...
//	$this->encounter['provider_id']='10';
    ...
    *****************************************************************/
    ...
    //At Atlas we are not using the Supervisor on the claim for. If it exists then it replaces the rendering provider on the claim.
    if($this->supervisor){

    $provider_id = $this->encounter['supervisor_id'];
    $sql = "SELECT * FROM users WHERE id = '$provider_id'";
    $this->provider = sqlQuery($sql);
    $this->billingIncidentTo=true;
    }
    ...
    //FOR MEDICARE ONLY USE BLOOMINGTON FOR THE BILLING FACILITY
    if ($this->payers[0]['company']['cms_id'] == 'MCRIL') {
      $sql = "SELECT * FROM facility " .
      " where id ='" . addslashes('3') . "' ";
...
error_log("Selected billing facility query: " . $sql);
    ...
    {
        $referrer_id=$billing_options_id;
    }
...
/////////
  ...
  function payerName($ins=0) {
    $payer = $this->payers[$ins];
    if($this->mcr_dme_claim=='1') $payer=$this->mcr_dme_payer;

    return x12clean(trim($payer['company']['name']));
  }
  ...
  function payerAttn($ins=0) {
    ...
    return x12clean(trim($payer['company']['attn']));
  }
  ...
  function payerID($ins=0) {
    ...
    return x12clean(trim($payer['company']['cms_id']));
  }
  ...
  function payerAltID($ins=0) {
    ...
    return x12clean(trim($payer['company']['alt_cms_id']));
  }
  ...
  function providerLastName($prockey=-1) {
    $tmp = ($this->billingIncidentTo || $prockey < 0 || empty($this->procs[$prockey]['provider_id'])) ?
